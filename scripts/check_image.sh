...
# Usage: bash check_image_phase3.sh [image]  (default vllm-node-glm53:v1-dflash2)
set -u
IMG="${1:-vllm-node-glm53:v1-dflash2}"
...
EXPECT_COMMIT="${EXPECT_COMMIT:-90104cfe4}"
...
# b12x whole-tree hash vs staged sparkinfer tree (py files only; order-stable)
tb=$(cd "$FIN/b12x" && LC_ALL=C find . -name '*.py' -type f | LC_ALL=C sort | xargs md5sum | md5sum | awk '{print $1}')
...
if [ "$tb" = "$ti" ]; then echo "PASS: b12x tree = staged sparkinfer @fefb9c5"; pass=$((pass+1))
else echo "FAIL: b12x tree mismatch (image=$ti host=$tb)"; fail=$((fail+1)); fi
echo "---"
